...
namespace DTcms.Web.UI.Page
{
    public partial class search : Web.UI.BasePage
    {
        ...
        protected string category_id = string.Empty;//频道名称
        ...
        protected override void ShowPage()
        {
            ...
            category_id = DTRequest.GetQueryString("category_id").Replace("'", string.Empty);
        }
        ...
        protected DataTable get_search_list(int _pagesize, out int _totalcount)
        {
            ...
            dt.Columns.Add("job_url", Type.GetType("System.String"));
            dt.Columns.Add("seo_title", Type.GetType("System.String"));
            dt.Columns.Add("click", Type.GetType("System.String"));
            //创建一个DataSet,判断是使用Tags还是关健字查询
            DataSet ds = new BLL.article().GetSearch(channel, category_id, _pagesize, page, "(title like '%" + keyword + "%' or zhaiyao like '%" + keyword + "%')", "add_time desc,id desc", out _totalcount);
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (!string.IsNullOrEmpty(link_url))
                    {
                        ...
                        dr["job_url"] = dr1["link_url"];//链接地址
                        dr["add_time"] = dr1["add_time"]; //发布时间
                        dr["channel_id"] = dr1["channel_id"]; //频道ID 
                        dr["category_id"] = dr1["category_id"]; //频道ID
                        dr["img_url"] = dr1["img_url"]; //发布时间
                        dr["seo_title"] = dr1["seo_title"];
                        dr["click"] = dr1["click"];
                        dt.Rows.Add(dr);
                    }
                }
